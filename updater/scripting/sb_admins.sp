...
public Action:Command_AddAdmin(client, args)
{
	if(args < 4)
	{
		ReplyToCommand(client, "%sUsage: sb_addadmin <name> <authtype> <identity> [password] [servergroup]", SB_PREFIX);
		return Plugin_Handled;
	}
	if(!SB_Connect())
	{
...
public Action:Command_SetAdminGroups(client, args)
{
	if(args < 2)
	{
		ReplyToCommand(client, "%sUsage: sb_setadmingroups <authtype> <identity> [servergroup]", SB_PREFIX);
		return Plugin_Handled;
	}
	if(!SB_Connect())
	{
